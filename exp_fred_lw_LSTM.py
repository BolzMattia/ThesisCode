...
if __name__ == '__main__':
    experiment_name = 'fred_lw_LSTM_1010'
    dataset_name = 'FRED'
    baseline_model = 'lw'
    hidden_layer_dim = 32
    batch_size = 16
    ...
    models = {
        'LW + VI-lstm': CholeskyLSTM(dsf,
                                            ...
                                            init_scale=2e0, learning_rate=5e-4),
        'LW + p-lstm': CholeskyLSTM(dsf,
                                             'x_timesteps', 'y', baseline_model_line,
                                             recurrent_dim=hidden_layer_dim,
                                             mlp_post_lstm_layers_dim=[hidden_layer_dim for i in range(2)],
                                             init_scale=1e0, learning_rate=3e-5),
        'LW + lstm': CholeskyLSTM(dsf,
                                   'x_timesteps', 'y', baseline_model_line,
                                   empirical_prior=False,
                                   recurrent_dim=hidden_layer_dim,
                                   mlp_post_lstm_layers_dim=[hidden_layer_dim for i in range(2)],
                                   init_scale=1e0, learning_rate=2e-5),
    }
